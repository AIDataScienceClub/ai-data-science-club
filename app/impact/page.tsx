...
import { loadPagesData } from '@/lib/data-loader'
...
export default async function Impact() {
  const pagesData = await loadPagesData() as Record<string, unknown> | null
  const pageData = pagesData?.impact as Record<string, unknown> | null
  
  const metrics2026: Metric[] = (pageData?.metrics as Metric[]) || [
  ...
  return (
    <>
      <Hero
        title={(pageData?.hero as Record<string, string>)?.title || "Measuring What Matters"}
        subtitle={(pageData?.hero as Record<string, string>)?.subtitle || "We hold ourselves accountable. Here's how we track access, learning, and long-term pathways—and what we've achieved so far."}
